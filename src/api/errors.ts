...
export class APIError {
  constructor(public type: ApiErrorType, public errors: ApiError[]) {}

  static fromAxiosError(error: AxiosError<ApiErrorResponse>): APIError {
    return new APIError(error.response!.data.type, error.response!.data.errors);
  }
  ...
  get attrs(): string[] {
    return this.errors.map((e) => e.attr).filter(Boolean) as string[];
  }
  ...
  getErrorByAttr(attr: string): ApiError | undefined {
    return this.errors.find((e) => e.attr === attr);
  }
}
